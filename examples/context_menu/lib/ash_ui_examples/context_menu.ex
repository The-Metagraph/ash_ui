defmodule AshUIExamples.ContextMenu do
  ...
  defmodule Web.ExampleLive do
    ...
    def mount(params, _session, socket) do
      ...
      socket =
        ...
        |> Phoenix.Component.assign(
          :supported_runtimes,
          AshUIExamples.ContextMenu.supported_runtimes()
        )
    ...
    def render(assigns) do
      assigns =
        ...
        |> Phoenix.Component.assign_new(:example_runtime, fn ->
          AshUIExamples.ContextMenu.default_runtime()
        end)
        |> Phoenix.Component.assign_new(:rendered_runtime, fn ->
          %{
            content: assigns[:rendered_ui] || "",
            description:
              AshUIExamples.ContextMenu.runtime_description(
                AshUIExamples.ContextMenu.default_runtime()
              ),
            mode: :live_fragment,
            runtime: AshUIExamples.ContextMenu.default_runtime()
          }
        end)
    ...
    defp refresh_rendered_ui(socket) do
      rendered_runtime =
        AshUIExamples.ContextMenu.rendered_runtime(
          socket.assigns,
          socket.assigns[:example_runtime] || AshUIExamples.ContextMenu.default_runtime()
        )
    ...
    defp runtime_from_params(params) do
      params["runtime"]
      |> fallback_runtime()
      |> AshUIExamples.ContextMenu.normalize_runtime!()
    end
