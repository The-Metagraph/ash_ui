defmodule AshUIExamples.Dialog do
  ...
  story_text: "Meaningful Interaction Story: confirm or cancel the dialog and verify that the result lands in persisted runtime state rather than living only inside ephemeral shell markup.",
  signal_text: "Canonical Signal Preview: nested button click -> ExampleState.selected_value and ExampleState.status -> dialog summary copy and preview stat.",
  seed_state: %{
    enabled: true,
    id: "state-dialog",
  ...
  support_notice: "The dialog shell stays explicit `custom:dialog` while the decision buttons own the action declarations.",
  ...
  def current_user, do: %{active: true, id: "reviewer-dialog", name: "Example Reviewer", role: :admin}
  def seed_state do
    Map.merge(
      %{
        id: "state-" <> @directory,
...
AshUIExamples.Dialog.Web.Endpoint
  ...
  defmodule Runtime.ExampleState do
    use Ash.Resource, domain: AshUIExamples.Dialog.RuntimeDomain, data_layer: Ash.DataLayer.Ets
  ...
  defmodule Examples.DialogSubjectElement do
    use AshUIExamples.Dialog.ExampleElementBase
          ...
          relationships do
            has_many :dialog_summary_elements, AshUIExamples.Dialog.Examples.DialogDialogSummaryElement do
              destination_attribute(:parent_id)
            end
            ...
            has_many :confirm_dialog_button_elements, AshUIExamples.Dialog.Examples.DialogConfirmDialogButtonElement do
              destination_attribute(:parent_id)
            end
            
            has_many :cancel_dialog_button_elements, AshUIExamples.Dialog.Examples.DialogCancelDialogButtonElement do
              destination_attribute(:parent_id)
            end
          end
      ...
      binding :dialog_open do
        ...
        defmodule Examples.DialogDialogSummaryElement do
          ...
          ui_element do
            ...
            props %{class: "ashui-example-surface-copy", content: "awaiting decision"}
  ...
  defmodule Examples.DialogStoryTextElement do
    ...
    ui_element do
      type :text
      props %{content: "Meaningful Interaction Story: confirm or cancel the dialog and verify that the result lands in persisted runtime state rather than living only inside ephemeral shell markup.", class: "ashui-example-code-surface"}
  ...
  defmodule Examples.DialogSignalTextElement do
    ...
    ui_element do
      type :text
      props %{content: "Canonical Signal Preview: nested button click -> ExampleState.selected_value and ExampleState.status -> dialog summary copy and preview stat.", class: "ashui-example-code-surface"}
      metadata %{id: "example-dialog-signal-preview", section: "signal_preview", slot: "body", position: 20}
    end
  end
  ...
  defmodule Examples.DialogSupportNoticeElement do
    ...
    ui_element do
      type :text
      props %{content: "The dialog shell stays explicit `custom:dialog` while the decision buttons own the action declarations.", class: "ashui-example-focus-ring"}
  ...
  defmodule Examples.DialogScreen do
    ...
    ui_screen do
      layout :column
      route "/"
      metadata %{title: "Dialog Example", example_directory: "dialog", shell_id: "example-dialog-shell"}
    end
  end
  ...
  defmodule Web.ExampleLive do
    ...
    defp refresh_rendered_ui(socket) do
      Phoenix.Component.assign(socket, :rendered_ui, AshUIExamples.Dialog.rendered_ui(socket.assigns))
    end
  end
end
