  ...
  defmodule Web.ExampleLive do
    ...
    def mount(params, _session, socket) do
      ...
      socket =
        ...
        |> Phoenix.Component.assign(:supported_runtimes, AshUIExamples.Grid.supported_runtimes())
    ...
    def render(assigns) do
      assigns =
        ...
        |> Phoenix.Component.assign_new(:supported_runtimes, fn ->
          AshUIExamples.Grid.supported_runtimes()
        end)
        |> Phoenix.Component.assign_new(:example_runtime, fn ->
          AshUIExamples.Grid.default_runtime()
        end)
        |> Phoenix.Component.assign_new(:rendered_runtime, fn ->
          %{
            content: assigns[:rendered_ui] || "",
            description:
              AshUIExamples.Grid.runtime_description(AshUIExamples.Grid.default_runtime()),
            mode: :live_fragment,
            runtime: AshUIExamples.Grid.default_runtime()
          }
        end)
    ...
    defp refresh_rendered_ui(socket) do
      rendered_runtime =
        AshUIExamples.Grid.rendered_runtime(
          socket.assigns,
          socket.assigns[:example_runtime] || AshUIExamples.Grid.default_runtime()
        )
    ...
    defp runtime_from_params(params) do
      params["runtime"]
      |> fallback_runtime()
      |> AshUIExamples.Grid.normalize_runtime!()
    end
