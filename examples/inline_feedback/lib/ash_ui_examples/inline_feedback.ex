  ...
  defmodule Web.ExampleLive do
    ...
    def mount(params, _session, socket) do
      ...
      socket =
        ...
        |> Phoenix.Component.assign(
          :supported_runtimes,
          AshUIExamples.InlineFeedback.supported_runtimes()
        )
    ...
    def render(assigns) do
      assigns =
        ...
        |> Phoenix.Component.assign_new(:example_runtime, fn ->
          AshUIExamples.InlineFeedback.default_runtime()
        end)
        |> Phoenix.Component.assign_new(:rendered_runtime, fn ->
          %{
            content: assigns[:rendered_ui] || "",
            description:
              AshUIExamples.InlineFeedback.runtime_description(
                AshUIExamples.InlineFeedback.default_runtime()
              ),
            mode: :live_fragment,
            runtime: AshUIExamples.InlineFeedback.default_runtime()
          }
        end)
    ...
    defp refresh_rendered_ui(socket) do
      rendered_runtime =
        AshUIExamples.InlineFeedback.rendered_runtime(
          socket.assigns,
          socket.assigns[:example_runtime] || AshUIExamples.InlineFeedback.default_runtime()
        )
    ...
    defp runtime_from_params(params) do
      params["runtime"]
      |> fallback_runtime()
      |> AshUIExamples.InlineFeedback.normalize_runtime!()
    end
