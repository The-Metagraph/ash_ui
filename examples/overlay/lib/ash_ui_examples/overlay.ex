defmodule AshUIExamples.Overlay do
  ...
  story_text: "Meaningful Interaction Story: open the overlay, inspect the layered body copy, and dismiss it again without losing the shared Ash HQ shell around the example.",
  signal_text: "Canonical Signal Preview: nested button click -> ExampleState.enabled -> overlay visibility and status copy inside the explicit `custom:overlay` shell.",
  seed_state: %{
    enabled: false,
    id: "state-overlay",
  ...
  support_notice: "The overlay shell remains explicit `custom:overlay`; open and dismiss semantics stay on nested public button resources.",
  ...
  def current_user, do: %{active: true, id: "reviewer-overlay", name: "Example Reviewer", role: :admin}
  def seed_state do
    Map.merge(
      %{
        id: "state-" <> @directory,
...
%{
  ...
  def reset! do
    reset_resource!(AshUIExamples.Overlay.Runtime.ExampleState, AshUIExamples.Overlay.RuntimeDomain)
...
AshUIExamples.Overlay.Web.Endpoint
  ...
  defmodule Runtime.ExampleState do
    use Ash.Resource, domain: AshUIExamples.Overlay.RuntimeDomain, data_layer: Ash.DataLayer.Ets
  ...
  defmodule Examples.OverlaySubjectElement do
    use AshUIExamples.Overlay.ExampleElementBase
          ...
          relationships do
            has_many :overlay_status_copy_elements, AshUIExamples.Overlay.Examples.OverlayOverlayStatusCopyElement do
              destination_attribute(:parent_id)
            end
            
            has_many :open_overlay_button_elements, AshUIExamples.Overlay.Examples.OverlayOpenOverlayButtonElement do
              destination_attribute(:parent_id)
            end
            
            has_many :dismiss_overlay_button_elements, AshUIExamples.Overlay.Examples.OverlayDismissOverlayButtonElement do
              destination_attribute(:parent_id)
            end
          end
  ...
  defmodule Examples.OverlayStoryTextElement do
    ...
    ui_element do
      type :text
      props %{content: "Meaningful Interaction Story: open the overlay, inspect the layered body copy, and dismiss it again without losing the shared Ash HQ shell around the example.", class: "ashui-example-code-surface"}
  ...
  defmodule Examples.OverlaySignalTextElement do
    ...
    ui_element do
      type :text
      props %{content: "Canonical Signal Preview: nested button click -> ExampleState.enabled -> overlay visibility and status copy inside the explicit `custom:overlay` shell.", class: "ashui-example-code-surface"}
      metadata %{id: "example-overlay-signal-preview", section: "signal_preview", slot: "body", position: 20}
    end
  end
  ...
  defmodule Examples.OverlaySupportNoticeElement do
    ...
    ui_element do
      type :text
      props %{content: "The overlay shell remains explicit `custom:overlay`; open and dismiss semantics stay on nested public button resources.", class: "ashui-example-focus-ring"}
  ...
  defmodule Examples.OverlayScreen do
    ...
    ui_screen do
      layout :column
      route "/"
      metadata %{title: "Overlay Example", example_directory: "overlay", shell_id: "example-overlay-shell"}
    end
  end
  ...
  defmodule Web.ExampleLive do
    ...
    defp refresh_rendered_ui(socket) do
      Phoenix.Component.assign(socket, :rendered_ui, AshUIExamples.Overlay.rendered_ui(socket.assigns))
    end
  end
end
