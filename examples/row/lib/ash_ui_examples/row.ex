  ...
  defmodule Web.ExampleLive do
    ...
    def mount(params, _session, socket) do
      ...
      socket =
        ...
        |> Phoenix.Component.assign(:supported_runtimes, AshUIExamples.Row.supported_runtimes())
    ...
    def render(assigns) do
      assigns =
        ...
        |> Phoenix.Component.assign_new(:supported_runtimes, fn ->
          AshUIExamples.Row.supported_runtimes()
        end)
        |> Phoenix.Component.assign_new(:example_runtime, fn ->
          AshUIExamples.Row.default_runtime()
        end)
        |> Phoenix.Component.assign_new(:rendered_runtime, fn ->
          %{
            content: assigns[:rendered_ui] || "",
            description:
              AshUIExamples.Row.runtime_description(AshUIExamples.Row.default_runtime()),
            mode: :live_fragment,
            runtime: AshUIExamples.Row.default_runtime()
          }
        end)
    ...
    defp refresh_rendered_ui(socket) do
      rendered_runtime =
        AshUIExamples.Row.rendered_runtime(
          socket.assigns,
          socket.assigns[:example_runtime] || AshUIExamples.Row.default_runtime()
        )
    ...
    defp runtime_from_params(params) do
      params["runtime"]
      |> fallback_runtime()
      |> AshUIExamples.Row.normalize_runtime!()
    end
