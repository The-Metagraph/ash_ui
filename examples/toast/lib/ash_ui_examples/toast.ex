defmodule AshUIExamples.Toast do
  @moduledoc """
  Standalone resource-authority Ash UI app for the `toast` example.
  """
  ...
  def ui_storage do
    [
      domain: AshUIExamples.Toast.UiStorageDomain,
      resources: [
        screen: AshUIExamples.Toast.UiScreen,
        element: AshUIExamples.Toast.UiElement,
        binding: AshUIExamples.Toast.UiBinding
      ],
      repo: nil
    ]
  end
  ...
  def reset! do
    reset_resource!(AshUIExamples.Toast.Runtime.ExampleState, AshUIExamples.Toast.RuntimeDomain)
    reset_resource!(AshUIExamples.Toast.UiBinding, AshUIExamples.Toast.UiStorageDomain)
    reset_resource!(AshUIExamples.Toast.UiElement, AshUIExamples.Toast.UiStorageDomain)
    reset_resource!(AshUIExamples.Toast.UiScreen, AshUIExamples.Toast.UiStorageDomain)
    :ok
  end
  ...
  defmodule RuntimeDomain do
    ...
    resources do
      resource(AshUIExamples.Toast.Runtime.ExampleState)
    end
  end
  ...
  defmodule UiStorageDomain do
    ...
    resources do
      resource(AshUIExamples.Toast.UiScreen)
      resource(AshUIExamples.Toast.UiElement)
      resource(AshUIExamples.Toast.UiBinding)
    end
  end
  ...
  defmodule UiScreen do
    ...
    relationships do
      has_many :elements, AshUIExamples.Toast.UiElement do
        destination_attribute(:screen_id)
      end

      has_many :bindings, AshUIExamples.Toast.UiBinding do
        destination_attribute(:screen_id)
      end
    end
  ...
  defmodule UiElement do
    ...
    relationships do
      belongs_to :screen, AshUIExamples.Toast.UiScreen do
        attribute_type(:uuid)
        allow_nil?(true)
      end
  ...
  defmodule UiBinding do
    ...
    relationships do
      belongs_to :element, AshUIExamples.Toast.UiElement do
        attribute_type(:uuid)
        allow_nil?(true)
      end
  ...
  defmodule ExampleElementBase do
    defmacro __using__(_opts) do
      quote do
        use Ash.Resource,
          domain: AshUIExamples.Toast.AuthoringDomain,
          data_layer: Ash.DataLayer.Ets
  ...
  defmodule Examples.ToastDemoPanelElement do
    use AshUIExamples.Toast.ExampleElementBase

    relationships do
      has_many :subjects, AshUIExamples.Toast.Examples.ToastSubjectElement do
        destination_attribute(:parent_id)
      end

      has_many :previews, AshUIExamples.Toast.Examples.ToastPreviewElement do
        destination_attribute(:parent_id)
      end
  ...
  defmodule Examples.ToastSignalTextElement do
    use AshUIExamples.Toast.ExampleElementBase
  ...
  defmodule Examples.ToastSupportNoticeElement do
    use AshUIExamples.Toast.ExampleElementBase
  ...
  defmodule Examples.ToastScreen do
    use Ash.Resource,
      domain: AshUIExamples.Toast.AuthoringDomain,
      data_layer: Ash.DataLayer.Ets
    ...
    relationships do
      has_many :demo_panels, AshUIExamples.Toast.Examples.ToastDemoPanelElement do
        destination_attribute(:screen_id)
      end

      has_many :story_texts, AshUIExamples.Toast.Examples.ToastStoryTextElement do
        destination_attribute(:screen_id)
      end

      has_many :signal_texts, AshUIExamples.Toast.Examples.ToastSignalTextElement do
        destination_attribute(:screen_id)
      end
    end
  ...
  defmodule ExampleSeeds do
    def seed!(opts \\ []), do: AshUIExamples.Toast.seed!(opts)
    def reset!, do: AshUIExamples.Toast.reset!()
  end
  ...
  defmodule Web.ExampleLive do
    ...
    alias AshUIExamples.Toast.Web.Components.ExampleShell
    alias AshUI.LiveView.EventHandler
    alias AshUI.LiveView.Integration

    def mount(params, _session, socket) do
      _ = AshUIExamples.Toast.seed!()
      example_runtime = runtime_from_params(params)

      socket =
        socket
        |> Phoenix.Component.assign(:current_user, AshUIExamples.Toast.current_user())
        |> Phoenix.Component.assign(:ash_ui_storage, AshUIExamples.Toast.ui_storage())
        |> Phoenix.Component.assign(:ash_ui_domains, AshUIExamples.Toast.runtime_domains())
        |> Phoenix.Component.assign(:page_title, "Toast Example")
        |> Phoenix.Component.assign(:example_directory, "toast")
        |> Phoenix.Component.assign(:theme_css, AshUIExamples.Toast.theme_css())
        |> Phoenix.Component.assign(:example_runtime, example_runtime)
        |> Phoenix.Component.assign(:supported_runtimes, AshUIExamples.Toast.supported_runtimes())

      with {:ok, socket} <- Integration.mount_ui_screen(socket, "example/toast", params),
    ...
    def render(assigns) do
      assigns =
        ...
        |> Phoenix.Component.assign_new(:supported_runtimes, fn ->
          AshUIExamples.Toast.supported_runtimes()
        end)
        |> Phoenix.Component.assign_new(:example_runtime, fn ->
          AshUIExamples.Toast.default_runtime()
        end)
        |> Phoenix.Component.assign_new(:rendered_runtime, fn ->
          %{
            content: assigns[:rendered_ui] || "",
            description:
              AshUIExamples.Toast.runtime_description(AshUIExamples.Toast.default_runtime()),
            mode: :live_fragment,
            runtime: AshUIExamples.Toast.default_runtime()
          }
        end)
      ...
      <ExampleShell.example_shell
        ...
        summary={"Meaningful Interaction Story: trigger different toast variants and confirm the visible message and status copy update through nested controls instead of hard-coded shell text."}
        theme_css={@theme_css}
      >
    ...
    defp refresh_rendered_ui(socket) do
      rendered_runtime =
        AshUIExamples.Toast.rendered_runtime(
          socket.assigns,
          socket.assigns[:example_runtime] || AshUIExamples.Toast.default_runtime()
        )
    ...
    defp runtime_from_params(params) do
      params["runtime"]
      |> fallback_runtime()
      |> AshUIExamples.Toast.normalize_runtime!()
    end
