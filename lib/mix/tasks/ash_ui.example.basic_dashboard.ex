defmodule Mix.Tasks.AshUi.Example.BasicDashboard do
  ...
  defp normalize_renderer!("html"), do: :html
  defp normalize_renderer!("web"), do: :html
  defp normalize_renderer!("desktop"), do: :desktop
  defp normalize_renderer!("native"), do: :desktop
  defp normalize_renderer!(other), do: raise(Mix.Error, message: "Unsupported renderer #{inspect(other)}.\n\n" <> usage())
  ...
  defp usage do
    """
    Usage:
      mix ash_ui.example.basic_dashboard --renderer liveview|html|desktop [--output PATH] [--pretty] [--strict-external]
    """
  end
end
