defmodule LiveUi.Widgets.WorkflowProgressStatusCard do
  @moduledoc """
  Native workflow-progress-status-card widget.

  Composite card representing a single workflow subject. Combines:
  ...
  - Dependency edges (depends-on / depended-by subject name lists)
  ...
  use LiveUi.Component,
    ...
    name: :workflow_progress_status_card,
    events: [:click]
  ...
  attr(:focus_intent, :string, default: "focus_subject")
  ...
  def render(assigns) do
    ...
    <article
      id={@id}
      data-live-ui-widget="workflow-progress-status-card"
      ...
      class={[
        "live-ui-workflow-progress-status-card",
        @selected? && "live-ui-workflow-progress-status-card--selected",
        @class
      ]}
      {@rest}
    >
      <header class="live-ui-workflow-progress-status-card__header">
        <button
          type="button"
          class="live-ui-workflow-progress-status-card__focus"
        ...
        >
          <span class="live-ui-workflow-progress-status-card__title"><%= @name %></span>
          <%= if @path do %>
            <span class="live-ui-workflow-progress-status-card__path"><%= @path %></span>
          <% end %>
        </button>
        <%= if @show_open_action && @open_action do %>
          <button
            type="button"
            class="live-ui-workflow-progress-status-card__open-action"
            phx-click="live_ui_interaction"
      ...
      <div
        class="live-ui-workflow-progress-status-card__progress-track"
        role="progressbar"
      ...
      >
        <div
          class="live-ui-workflow-progress-status-card__progress-fill"
          style={"width: #{@progress_pct_int}%"}
        >
        </div>
      </div>

      <div class="live-ui-workflow-progress-status-card__stats">
        <span class="live-ui-workflow-progress-status-card__stat-chip">
          <%= @active_count %> active
        </span>
        <span
          class="live-ui-workflow-progress-status-card__stat-chip"
      ...
      <%= if @depends_on != [] do %>
        <div class="live-ui-workflow-progress-status-card__deps live-ui-workflow-progress-status-card__deps--depends-on">
          depends on: <%= Enum.join(@depends_on, ", ") %>
        </div>
      <% end %>
      <%= if @depended_by != [] do %>
        <div class="live-ui-workflow-progress-status-card__deps live-ui-workflow-progress-status-card__deps--depended-by">
