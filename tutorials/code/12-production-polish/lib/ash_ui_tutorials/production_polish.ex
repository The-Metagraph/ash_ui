defmodule AshUITutorials.ProductionPolish do
  @moduledoc """
  Standalone Chapter 12 checkpoint app for the Operations Control Center tutorial.
  """
  ...
  @app :ash_ui_tutorial_production_polish
  @screen_names %{
  ...
  @summary "Standalone Chapter 12 checkpoint app: the earlier filters, workflows, runbook review, seeded diagnostics, topology navigation, metrics dashboards, runtime-introspection surfaces, and actor-aware policies remain intact while the tutorial now adds explicit ready/loading/empty/error review states, more accessible shell navigation, and responsive polish that keeps the final teaching surface coherent on desktop and mobile."
  ...
  def runtime_domains, do: [AshUITutorials.ProductionPolish.RuntimeDomain]
  ...
  defp gateway_runtime_context do
    %{
      runtime_focus: "gateway supervisor",
      runtime_query: "",
      runtime_supervision_model: %{
        "label" => "Gateway supervisor",
        "meta" => "Core East ingress lane",
        "nodes" => [
          %{"label" => "gateway_acceptor_supervisor", "meta" => "running"},
          %{
            "label" => "gateway_retry_supervisor",
            "meta" => "watching",
            "children" => [
              %{"label" => "retry_worker_1", "meta" => "1 restart"},
              %{"label" => "retry_worker_2", "meta" => "0 restarts"}
            ]
          },
          %{"label" => "gateway_binding_refresh", "meta" => "idle"}
        ]
      },
      runtime_process_catalog: [
        %{"component" => "gateway_acceptor_supervisor", "state" => "running", "detail" => "Holding the public ingress pool steady."},
        %{"component" => "gateway_retry_supervisor", "state" => "watching", "detail" => "Monitoring retry pressure after the latency spike."},
        %{"component" => "retry_worker_1", "state" => "running", "detail" => "Observed one restart in the current review window."},
        %{"component" => "gateway_binding_refresh", "state" => "idle", "detail" => "Waiting for the next persistence refresh cycle."}
      ],
      runtime_support_title: "Gateway supervisor lane",
      runtime_support_notice:
        "This runtime tree is tutorial-seeded: the labels and restart counts are representative operator-facing snapshots, not a hidden live supervisor tap.",
      runtime_status_copy:
        "Runtime review is focused on the gateway supervisor lane and the retry workers behind the sev-1 ingress path."
    }
  end

  defp search_runtime_context do
    %{
      runtime_focus: "search replica supervision",
      runtime_query: "",
      runtime_supervision_model: %{
        "label" => "Search replica supervisor",
        "meta" => "Edge West recovery lane",
        "nodes" => [
          %{"label" => "replica_reader_supervisor", "meta" => "degraded"},
          %{
            "label" => "replica_backlog_workers",
            "meta" => "recovering",
            "children" => [
              %{"label" => "backlog_shard_a", "meta" => "running"},
              %{"label" => "backlog_shard_b", "meta" => "queued"}
            ]
          },
          %{"label" => "promotion_guard", "meta" => "idle"}
        ]
      },
      runtime_process_catalog: [
        %{"component" => "replica_reader_supervisor", "state" => "degraded", "detail" => "Lag remains elevated but trending down."},
        %{"component" => "backlog_shard_a", "state" => "running", "detail" => "Current recovery shard is draining normally."},
        %{"component" => "backlog_shard_b", "state" => "queued", "detail" => "Queued until replica lag falls under the next threshold."},
        %{"component" => "promotion_guard", "state" => "idle", "detail" => "Waiting for the final lag and freshness checks."}
      ],
      runtime_support_title: "Replica recovery lane",
      runtime_support_notice:
        "This view stays explicit about being sampled: the backlog workers and promotion guard represent the operator's captured review packet, not a direct runtime probe.",
      runtime_status_copy:
        "Runtime review is narrowed to the search recovery supervisor and the queued backlog workers."
    }
  end

  defp recovery_runtime_context do
    %{
      runtime_focus: "rollback coordination",
      runtime_query: "",
      runtime_supervision_model: %{
        "label" => "Rollback coordinator",
        "meta" => "Recovery orchestration",
        "nodes" => [
          %{"label" => "rollback_packet_supervisor", "meta" => "running"},
          %{
            "label" => "broadcast_supervisor",
            "meta" => "running",
            "children" => [
              %{"label" => "notify_commander", "meta" => "sent"},
              %{"label" => "notify_platform_manager", "meta" => "queued"}
            ]
          },
          %{"label" => "maintenance_window_guard", "meta" => "active"}
        ]
      },
      runtime_process_catalog: [
        %{"component" => "rollback_packet_supervisor", "state" => "running", "detail" => "Coordinating the current rollback review packet."},
        %{"component" => "notify_commander", "state" => "sent", "detail" => "Incident command already received the first rollback note."},
        %{"component" => "notify_platform_manager", "state" => "queued", "detail" => "Queued behind the maintenance-window confirmation."},
        %{"component" => "maintenance_window_guard", "state" => "active", "detail" => "Blocking restart until the maintenance plan is complete."}
      ],
      runtime_support_title: "Recovery coordination lane",
      runtime_support_notice:
        "This recovery tree is deliberately operator-shaped. It teaches how to present rollback coordination honestly without implying a hidden orchestration runtime inside the tutorial app.",
      runtime_status_copy:
        "Runtime review is focused on the rollback coordination lane and the guarded recovery notifications behind it."
    }
  end
  ...
  def hydrate_state(attrs) do
    attrs =
      ...
      |> Map.put_new(:runtime_focus, gateway_runtime_context().runtime_focus)
      |> Map.put_new(:runtime_query, gateway_runtime_context().runtime_query)
      |> Map.put_new(:runtime_supervision_model, gateway_runtime_context().runtime_supervision_model)
      |> Map.put_new(:runtime_process_catalog, gateway_runtime_context().runtime_process_catalog)
    ...
    |> Map.put(:runtime_process_rows, visible_runtime_process_rows)
    |> Map.put(:runtime_command_summary, runtime_command_summary(attrs, visible_runtime_process_rows))
  ...
  def seed_state(actor \\ current_user()) do
    ...
    hydrate_state(%{
      ...
      runtime_focus: gateway_runtime_context().runtime_focus,
      runtime_query: gateway_runtime_context().runtime_query,
      runtime_supervision_model: gateway_runtime_context().runtime_supervision_model,
  ...
  def reset! do
    reset_resource!(
      ...
      AshUITutorials.ProductionPolish.RuntimeDomain
    )

    reset_resource!(
      AshUITutorials.ProductionPolish.UiBinding,
      AshUITutorials.ProductionPolish.UiStorageDomain
    )

    reset_resource!(
      AshUITutorials.ProductionPolish.UiElement,
      AshUITutorials.ProductionPolish.UiStorageDomain
    )

    reset_resource!(
      AshUITutorials.ProductionPolish.UiScreen,
      AshUITutorials.ProductionPolish.UiStorageDomain
    )
  ...
  def seed!(opts \\ []) do
    ...
    {:ok, _state} =
      Ash.create(
        ...
        domain: AshUITutorials.ProductionPolish.RuntimeDomain,
        authorize?: false
      )

    {:ok, services_screen} =
      Authority.create(
        AshUITutorials.ProductionPolish.Examples.ServicesScreen,
    ...
    {:ok, incidents_screen} =
      Authority.create(
        AshUITutorials.ProductionPolish.Examples.IncidentsScreen,
        actor: authoring_actor,
  ...
  defp runtime_command_summary(attrs, visible_runtime_process_rows) do
    query = blank_to_phrase(attrs[:runtime_query], "all runtime rows")

    "#{attrs[:runtime_focus]} is active; runtime query=#{query}; #{length(visible_runtime_process_rows)} process rows remain visible."
  end

  defp filter_runtime_process_rows(rows, query) when query in [nil, ""] do
    rows
  end

  defp filter_runtime_process_rows(rows, query) do
  ...
  defmodule Application do
    ...
    def start(_type, _args) do
      children = [
        {Phoenix.PubSub, name: AshUITutorials.ProductionPolish.PubSub},
        AshUITutorials.ProductionPolish.Web.Endpoint
      ]
  ...
  defmodule RuntimeDomain do
    ...
    resources do
      resource(AshUITutorials.ProductionPolish.Runtime.WorkspaceState)
    end
  end
  ...
  defmodule Runtime.WorkspaceState do
    ...
    attributes do
      ...
      attribute :runtime_focus, :string, default: ""
      attribute :runtime_query, :string, default: ""
      attribute :runtime_supervision_model, :map, default: %{}
      attribute :runtime_process_catalog, {:array, :map}, default: []
      attribute :runtime_process_rows, {:array, :map}, default: []
      attribute :runtime_command_summary, :string, default: ""
    ...
    defp hydrate_changeset(changeset) do
      changeset
      |> state_attrs_from()
      |> AshUITutorials.ProductionPolish.hydrate_state()
  ...
  defmodule UiStorageDomain do
    ...
    resources do
      resource(AshUITutorials.ProductionPolish.UiScreen)
      resource(AshUITutorials.ProductionPolish.UiElement)
      resource(AshUITutorials.ProductionPolish.UiBinding)
    end
  end
  ...
  defmodule UiScreen do
    ...
    relationships do
      has_many :elements, AshUITutorials.ProductionPolish.UiElement do
        destination_attribute(:screen_id)
      end

      has_many :bindings, AshUITutorials.ProductionPolish.UiBinding do
        destination_attribute(:screen_id)
      end
    end
  ...
  defmodule UiElement do
    ...
    relationships do
      belongs_to :screen, AshUITutorials.ProductionPolish.UiScreen do
        attribute_type(:uuid)
        allow_nil?(true)
      end
  ...
  defmodule UiBinding do
    ...
    relationships do
      belongs_to :element, AshUITutorials.ProductionPolish.UiElement do
        attribute_type(:uuid)
        allow_nil?(true)
      end
  ...
  defmodule AuthoringDomain do
    ...
    resources do
      resource(AshUITutorials.ProductionPolish.Examples.ServicesScreen)
      resource(AshUITutorials.ProductionPolish.Examples.ServicesWorkspacePanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.WorkspaceMenuElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowServicesButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowIncidentsButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowOperatorViewButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.WorkspaceSelectionSummaryElement)
      resource(AshUITutorials.ProductionPolish.Examples.CommandPaletteElement)
      resource(AshUITutorials.ProductionPolish.Examples.CommandPaletteInputElement)
      resource(AshUITutorials.ProductionPolish.Examples.CommandFocusGatewayButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.CommandFocusIncidentButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.CommandOpenOperatorViewButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.CommandSummaryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServicesProductionPolishPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentsProductionPolishPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.ReviewStateStatusElement)
      resource(AshUITutorials.ProductionPolish.Examples.ReviewStateInlineFeedbackElement)
      resource(AshUITutorials.ProductionPolish.Examples.ProductionPolishGuidanceTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowServicesReadyStateButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowServicesLoadingStateButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowServicesEmptyStateButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowServicesErrorStateButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowIncidentsReadyStateButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowIncidentsLoadingStateButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowIncidentsEmptyStateButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowIncidentsErrorStateButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServicesFiltersGroupElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServicesQueryFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServicesQueryInputElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServiceStatusFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServiceStatusSelectElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncludeHealthyFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncludeHealthyCheckboxElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServicesListElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyReviewPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologySplitPaneElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyScopeMenuElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowServiceTopologyButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowDependencyTopologyButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowIncidentScopeTopologyButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyScopeSummaryElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyTabsElement)
      resource(AshUITutorials.ProductionPolish.Examples.FocusGatewayTopologyTabButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.FocusSearchTopologyTabButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.FocusClusterTopologyTabButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyTabsStatusElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyTreeViewElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyViewportElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyViewportFocusCopyElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyViewportSupportPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyViewportSupportTitleElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyViewportSupportDetailElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyCanvasElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyCanvasTrafficPathButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyCanvasBlastRadiusButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyCanvasLayerElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyCanvasBoardCopyElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyCanvasLegendElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyScrollBarElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyCommanderScrollButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyInfrastructureScrollButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyHandoffScrollButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyScrollFocusCopyElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyScrollStatusElement)
      resource(AshUITutorials.ProductionPolish.Examples.TopologyPanelStatusTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsReviewPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsClusterDashboardElement)
      resource(AshUITutorials.ProductionPolish.Examples.LoadGatewayMetricsButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.LoadSearchMetricsButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.LoadFleetMetricsButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsDashboardFooterElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsProgressElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsGaugeElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsSparklineElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsBarChartElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsLineChartElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsSupportNoticeElement)
      resource(AshUITutorials.ProductionPolish.Examples.MetricsPanelStatusTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeReviewPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeCommandPaletteElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeCommandSearchInputElement)
      resource(AshUITutorials.ProductionPolish.Examples.FocusGatewayRuntimeButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.FocusSearchRuntimeButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.FocusRecoveryRuntimeButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeCommandSummaryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeTabsElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowGatewayRuntimeTabButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowSearchRuntimeTabButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ShowRecoveryRuntimeTabButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeTabsStatusTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeSupportPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeSupportTitleElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeSupportNoticeElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeSupervisionTreeViewerElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeProcessTableElement)
      resource(AshUITutorials.ProductionPolish.Examples.RuntimeStatusTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.RolePolicySummaryPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.RolePolicyNameTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.RolePolicySummaryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.RolePolicyNoticeTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.SharedDetailCardElement)
      resource(AshUITutorials.ProductionPolish.Examples.SharedDetailBadgeElement)
      resource(AshUITutorials.ProductionPolish.Examples.SharedDetailTitleElement)
      resource(AshUITutorials.ProductionPolish.Examples.SharedDetailSummaryElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServicesStatusTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServicesStoryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.ServicesSignalTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentsScreen)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentsWorkspacePanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentsFiltersGroupElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentSeverityFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentSeverityRadioElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentEscalatedFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentEscalatedSwitchElement)
      resource(AshUITutorials.ProductionPolish.Examples.OperatorFormsPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.OperatorWorkflowFormElement)
      resource(AshUITutorials.ProductionPolish.Examples.NoteAndAssignmentGroupElement)
      resource(AshUITutorials.ProductionPolish.Examples.OperatorNoteFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.OperatorNoteInputElement)
      resource(AshUITutorials.ProductionPolish.Examples.AssignmentTargetFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.AssignmentTargetPickListElement)
      resource(AshUITutorials.ProductionPolish.Examples.MaintenanceWindowGroupElement)
      resource(AshUITutorials.ProductionPolish.Examples.MaintenanceDurationFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.MaintenanceDurationInputElement)
      resource(AshUITutorials.ProductionPolish.Examples.MaintenanceDateFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.MaintenanceDateInputElement)
      resource(AshUITutorials.ProductionPolish.Examples.MaintenanceTimeFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.MaintenanceTimeInputElement)
      resource(AshUITutorials.ProductionPolish.Examples.AcknowledgeIncidentButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.AssignIncidentButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ScheduleMaintenanceButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.FormFeedbackBadgeElement)
      resource(AshUITutorials.ProductionPolish.Examples.FormFeedbackTitleElement)
      resource(AshUITutorials.ProductionPolish.Examples.FormFeedbackSummaryElement)
      resource(AshUITutorials.ProductionPolish.Examples.GuardedActionsPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.GuardedActionsMenuElement)
      resource(AshUITutorials.ProductionPolish.Examples.OpenResolveGuardButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.OpenRestartGuardButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.OpenSilenceGuardButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.OpenDiscardNoteGuardButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.GuardSummaryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.GuardOverlayElement)
      resource(AshUITutorials.ProductionPolish.Examples.GuardOverlayTitleTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.GuardOverlaySummaryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.ConfirmOverlayGuardButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.CancelGuardSurfaceButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.ResolveGuardDialogElement)
      resource(AshUITutorials.ProductionPolish.Examples.ResolveGuardSummaryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.ConfirmResolveGuardButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.RestartGuardAlertElement)
      resource(AshUITutorials.ProductionPolish.Examples.RestartGuardSummaryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.ConfirmRestartGuardButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.GuardResultToastElement)
      resource(AshUITutorials.ProductionPolish.Examples.GuardToastTitleTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.GuardToastSummaryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.DismissGuardToastButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentsTableElement)
      resource(AshUITutorials.ProductionPolish.Examples.RunbookReviewPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.RunbookSplitPaneElement)
      resource(AshUITutorials.ProductionPolish.Examples.RunbookFocusTitleElement)
      resource(AshUITutorials.ProductionPolish.Examples.RunbookMarkdownViewerElement)
      resource(AshUITutorials.ProductionPolish.Examples.LoadGatewayRunbookButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.LoadRollbackRunbookButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.AttachmentEvidenceCardElement)
      resource(AshUITutorials.ProductionPolish.Examples.AttachmentPreviewTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.AttachmentFileFieldElement)
      resource(AshUITutorials.ProductionPolish.Examples.AttachmentFileInputElement)
      resource(AshUITutorials.ProductionPolish.Examples.AttachmentReferenceLinkElement)
      resource(AshUITutorials.ProductionPolish.Examples.AttachmentImageElement)
      resource(AshUITutorials.ProductionPolish.Examples.AttachmentSupportTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.RunbookStatusTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.LiveDiagnosticsPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.LoadGatewayDiagnosticsButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.LoadSearchDiagnosticsButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.LoadPressureDiagnosticsButtonElement)
      resource(AshUITutorials.ProductionPolish.Examples.DiagnosticsStatusElement)
      resource(AshUITutorials.ProductionPolish.Examples.DiagnosticsInlineFeedbackElement)
      resource(AshUITutorials.ProductionPolish.Examples.DiagnosticsLogViewerElement)
      resource(AshUITutorials.ProductionPolish.Examples.DiagnosticsStreamWidgetElement)
      resource(AshUITutorials.ProductionPolish.Examples.DiagnosticsProcessMonitorElement)
      resource(AshUITutorials.ProductionPolish.Examples.DiagnosticsStatusTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.AdminPolicyAuditPanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.AdminPolicyAuditTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.ViewerPolicyNoticePanelElement)
      resource(AshUITutorials.ProductionPolish.Examples.ViewerPolicyNoticeTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentsStatusTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentsStoryTextElement)
      resource(AshUITutorials.ProductionPolish.Examples.IncidentsSignalTextElement)
    end
  end
  ...
  defmodule ExampleElementBase do
    defmacro __using__(_opts) do
      quote do
        use Ash.Resource,
          domain: AshUITutorials.ProductionPolish.AuthoringDomain,
          data_layer: Ash.DataLayer.Ets
  ...
  defmodule Examples.ServicesWorkspacePanelElement do
    use AshUITutorials.ProductionPolish.ExampleElementBase

    relationships do
      has_many :menus, AshUITutorials.ProductionPolish.Examples.WorkspaceMenuElement do
        destination_attribute(:parent_id)
      end

      has_many :command_palettes,
               AshUITutorials.ProductionPolish.Examples.CommandPaletteElement do
        destination_attribute(:parent_id)
      end

      has_many :filter_groups,
               AshUITutorials.ProductionPolish.Examples.ServicesFiltersGroupElement do
        destination_attribute(:parent_id)
      end

      has_many :production_polish_panels,
               AshUITutorials.ProductionPolish.Examples.ServicesProductionPolishPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :service_lists, AshUITutorials.ProductionPolish.Examples.ServicesListElement do
        destination_attribute(:parent_id)
      end

      has_many :topology_review_panels,
               AshUITutorials.ProductionPolish.Examples.TopologyReviewPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :metrics_review_panels,
               AshUITutorials.ProductionPolish.Examples.MetricsReviewPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :runtime_review_panels,
               AshUITutorials.ProductionPolish.Examples.RuntimeReviewPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :role_policy_panels,
               AshUITutorials.ProductionPolish.Examples.RolePolicySummaryPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :detail_cards, AshUITutorials.ProductionPolish.Examples.SharedDetailCardElement do
        destination_attribute(:parent_id)
      end

      has_many :status_texts, AshUITutorials.ProductionPolish.Examples.ServicesStatusTextElement do
        destination_attribute(:parent_id)
      end
    end
    ...
    ui_relationships do
      ...
      relationship :production_polish_panels do
        kind(:child)
        slot(:body)
        placement(:append)
        order(25)
      ...
      relationship :runtime_review_panels do
        kind(:child)
        slot(:body)
        placement(:append)
        order(60)
  ...
  defmodule Examples.WorkspaceMenuElement do
    ...
    relationships do
      has_many :services_buttons, AshUITutorials.ProductionPolish.Examples.ShowServicesButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :incidents_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowIncidentsButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :operator_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowOperatorViewButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :selection_summaries,
               AshUITutorials.ProductionPolish.Examples.WorkspaceSelectionSummaryElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.CommandPaletteElement do
    ...
    relationships do
      has_many :search_inputs,
               AshUITutorials.ProductionPolish.Examples.CommandPaletteInputElement do
        destination_attribute(:parent_id)
      end

      has_many :gateway_buttons,
               AshUITutorials.ProductionPolish.Examples.CommandFocusGatewayButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :incident_buttons,
               AshUITutorials.ProductionPolish.Examples.CommandFocusIncidentButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :operator_buttons,
               AshUITutorials.ProductionPolish.Examples.CommandOpenOperatorViewButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :summary_texts,
               AshUITutorials.ProductionPolish.Examples.CommandSummaryTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.ServicesProductionPolishPanelElement do
    use AshUITutorials.ProductionPolish.ExampleElementBase

    relationships do
      has_many :ready_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowServicesReadyStateButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :loading_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowServicesLoadingStateButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :empty_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowServicesEmptyStateButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :error_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowServicesErrorStateButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :status_widgets,
               AshUITutorials.ProductionPolish.Examples.ReviewStateStatusElement do
        destination_attribute(:parent_id)
      end

      has_many :feedback_widgets,
               AshUITutorials.ProductionPolish.Examples.ReviewStateInlineFeedbackElement do
        destination_attribute(:parent_id)
      end

      has_many :guidance_texts,
               AshUITutorials.ProductionPolish.Examples.ProductionPolishGuidanceTextElement do
        destination_attribute(:parent_id)
      end
    end
    ...
    ui_element do
      ...
      metadata(%{id: "services-production-polish-panel", section: "demo", slot: "body", position: 25})
    end
  end

  defmodule Examples.IncidentsProductionPolishPanelElement do
    use AshUITutorials.ProductionPolish.ExampleElementBase

    relationships do
      has_many :ready_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowIncidentsReadyStateButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :loading_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowIncidentsLoadingStateButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :empty_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowIncidentsEmptyStateButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :error_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowIncidentsErrorStateButtonElement do
        destination_attribute(:parent_id)
      end
    ...
    ui_element do
      ...
      metadata(%{id: "incidents-production-polish-panel", section: "demo", slot: "body", position: 15})
    end
  end
  ...
  defmodule Examples.ProductionPolishGuidanceTextElement do
    use AshUITutorials.ProductionPolish.ExampleElementBase
    ...
    ui_element do
      ...
      metadata(%{id: "production-polish-guidance", section: "demo", slot: "footer", position: 0})
    end
  end
  ...
  defmodule Examples.ServicesFiltersGroupElement do
    ...
    relationships do
      has_many :query_fields,
               AshUITutorials.ProductionPolish.Examples.ServicesQueryFieldElement do
        destination_attribute(:parent_id)
      end

      has_many :status_fields,
               AshUITutorials.ProductionPolish.Examples.ServiceStatusFieldElement do
        destination_attribute(:parent_id)
      end

      has_many :healthy_fields,
               AshUITutorials.ProductionPolish.Examples.IncludeHealthyFieldElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.ServicesQueryFieldElement do
    ...
    relationships do
      has_many :inputs,
               AshUITutorials.ProductionPolish.Examples.ServicesQueryInputElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.ServiceStatusFieldElement do
    ...
    relationships do
      has_many :selects,
               AshUITutorials.ProductionPolish.Examples.ServiceStatusSelectElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.IncludeHealthyFieldElement do
    ...
    relationships do
      has_many :checkboxes,
               AshUITutorials.ProductionPolish.Examples.IncludeHealthyCheckboxElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.TopologyReviewPanelElement do
    ...
    relationships do
      has_many :split_panes, AshUITutorials.ProductionPolish.Examples.TopologySplitPaneElement do
        destination_attribute(:parent_id)
      end

      has_many :status_texts,
               AshUITutorials.ProductionPolish.Examples.TopologyPanelStatusTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.TopologySplitPaneElement do
    ...
    relationships do
      has_many :scope_menus,
               AshUITutorials.ProductionPolish.Examples.TopologyScopeMenuElement do
        destination_attribute(:parent_id)
      end

      has_many :tabs, AshUITutorials.ProductionPolish.Examples.TopologyTabsElement do
        destination_attribute(:parent_id)
      end

      has_many :tree_views,
               AshUITutorials.ProductionPolish.Examples.TopologyTreeViewElement do
        destination_attribute(:parent_id)
      end

      has_many :viewports,
               AshUITutorials.ProductionPolish.Examples.TopologyViewportElement do
        destination_attribute(:parent_id)
      end

      has_many :canvases,
               AshUITutorials.ProductionPolish.Examples.TopologyCanvasElement do
        destination_attribute(:parent_id)
      end

      has_many :scroll_bars,
               AshUITutorials.ProductionPolish.Examples.TopologyScrollBarElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.TopologyScopeMenuElement do
    ...
    relationships do
      has_many :service_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowServiceTopologyButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :dependency_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowDependencyTopologyButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :incident_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowIncidentScopeTopologyButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :scope_summaries,
               AshUITutorials.ProductionPolish.Examples.TopologyScopeSummaryElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.TopologyTabsElement do
    ...
    relationships do
      has_many :gateway_buttons,
               AshUITutorials.ProductionPolish.Examples.FocusGatewayTopologyTabButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :search_buttons,
               AshUITutorials.ProductionPolish.Examples.FocusSearchTopologyTabButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :cluster_buttons,
               AshUITutorials.ProductionPolish.Examples.FocusClusterTopologyTabButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :status_texts,
               AshUITutorials.ProductionPolish.Examples.TopologyTabsStatusElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.TopologyViewportElement do
    ...
    relationships do
      has_many :focus_copy_elements,
               AshUITutorials.ProductionPolish.Examples.TopologyViewportFocusCopyElement do
        destination_attribute(:parent_id)
      end

      has_many :support_panels,
               AshUITutorials.ProductionPolish.Examples.TopologyViewportSupportPanelElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.TopologyViewportSupportPanelElement do
    ...
    relationships do
      has_many :titles,
               AshUITutorials.ProductionPolish.Examples.TopologyViewportSupportTitleElement do
        destination_attribute(:parent_id)
      end

      has_many :details,
               AshUITutorials.ProductionPolish.Examples.TopologyViewportSupportDetailElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.TopologyCanvasElement do
    ...
    relationships do
      has_many :traffic_buttons,
               AshUITutorials.ProductionPolish.Examples.TopologyCanvasTrafficPathButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :blast_buttons,
               AshUITutorials.ProductionPolish.Examples.TopologyCanvasBlastRadiusButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :layer_elements,
               AshUITutorials.ProductionPolish.Examples.TopologyCanvasLayerElement do
        destination_attribute(:parent_id)
      end

      has_many :board_copy_elements,
               AshUITutorials.ProductionPolish.Examples.TopologyCanvasBoardCopyElement do
        destination_attribute(:parent_id)
      end

      has_many :legend_elements,
               AshUITutorials.ProductionPolish.Examples.TopologyCanvasLegendElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.TopologyScrollBarElement do
    ...
    relationships do
      has_many :focus_copy_elements,
               AshUITutorials.ProductionPolish.Examples.TopologyScrollFocusCopyElement do
        destination_attribute(:parent_id)
      end

      has_many :commander_buttons,
               AshUITutorials.ProductionPolish.Examples.TopologyCommanderScrollButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :infrastructure_buttons,
               AshUITutorials.ProductionPolish.Examples.TopologyInfrastructureScrollButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :handoff_buttons,
               AshUITutorials.ProductionPolish.Examples.TopologyHandoffScrollButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :status_texts,
               AshUITutorials.ProductionPolish.Examples.TopologyScrollStatusElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.MetricsReviewPanelElement do
    ...
    relationships do
      has_many :cluster_dashboards,
               AshUITutorials.ProductionPolish.Examples.MetricsClusterDashboardElement do
        destination_attribute(:parent_id)
      end

      has_many :progress_surfaces,
               AshUITutorials.ProductionPolish.Examples.MetricsProgressElement do
        destination_attribute(:parent_id)
      end

      has_many :gauge_surfaces,
               AshUITutorials.ProductionPolish.Examples.MetricsGaugeElement do
        destination_attribute(:parent_id)
      end

      has_many :sparkline_surfaces,
               AshUITutorials.ProductionPolish.Examples.MetricsSparklineElement do
        destination_attribute(:parent_id)
      end

      has_many :bar_chart_surfaces,
               AshUITutorials.ProductionPolish.Examples.MetricsBarChartElement do
        destination_attribute(:parent_id)
      end

      has_many :line_chart_surfaces,
               AshUITutorials.ProductionPolish.Examples.MetricsLineChartElement do
        destination_attribute(:parent_id)
      end

      has_many :support_notices,
               AshUITutorials.ProductionPolish.Examples.MetricsSupportNoticeElement do
        destination_attribute(:parent_id)
      end

      has_many :status_texts,
               AshUITutorials.ProductionPolish.Examples.MetricsPanelStatusTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.MetricsClusterDashboardElement do
    ...
    relationships do
      has_many :gateway_buttons,
               AshUITutorials.ProductionPolish.Examples.LoadGatewayMetricsButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :search_buttons,
               AshUITutorials.ProductionPolish.Examples.LoadSearchMetricsButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :fleet_buttons,
               AshUITutorials.ProductionPolish.Examples.LoadFleetMetricsButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :footer_elements,
               AshUITutorials.ProductionPolish.Examples.MetricsDashboardFooterElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.RuntimeReviewPanelElement do
    ...
    relationships do
      has_many :command_palettes,
               AshUITutorials.ProductionPolish.Examples.RuntimeCommandPaletteElement do
        destination_attribute(:parent_id)
      end

      has_many :tabs, AshUITutorials.ProductionPolish.Examples.RuntimeTabsElement do
        destination_attribute(:parent_id)
      end

      has_many :support_panels,
               AshUITutorials.ProductionPolish.Examples.RuntimeSupportPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :supervision_viewers,
               AshUITutorials.ProductionPolish.Examples.RuntimeSupervisionTreeViewerElement do
        destination_attribute(:parent_id)
      end

      has_many :process_tables,
               AshUITutorials.ProductionPolish.Examples.RuntimeProcessTableElement do
        destination_attribute(:parent_id)
      end

      has_many :status_texts,
               AshUITutorials.ProductionPolish.Examples.RuntimeStatusTextElement do
        destination_attribute(:parent_id)
      end
    end
    ...
    ui_relationships do
      ...
      relationship :supervision_viewers do
        kind(:child)
        slot(:body)
        placement(:append)
        order(30)
      ...
      relationship :process_tables do
        kind(:child)
        slot(:body)
        placement(:append)
        order(40)
    ...
    ui_element do
      ...
      props(%{
        ...
        class: "ashui-example-panel ashui-tutorial-workspace-panel ashui-tutorial-runtime-panel"
      })

      metadata(%{id: "runtime-review-panel", section: "demo", slot: "body", position: 60})
    end
  end
  ...
  defmodule Examples.RuntimeCommandPaletteElement do
    ...
    relationships do
      has_many :search_inputs,
               AshUITutorials.ProductionPolish.Examples.RuntimeCommandSearchInputElement do
        destination_attribute(:parent_id)
      end

      has_many :gateway_buttons,
               AshUITutorials.ProductionPolish.Examples.FocusGatewayRuntimeButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :search_buttons,
               AshUITutorials.ProductionPolish.Examples.FocusSearchRuntimeButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :recovery_buttons,
               AshUITutorials.ProductionPolish.Examples.FocusRecoveryRuntimeButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :summary_texts,
               AshUITutorials.ProductionPolish.Examples.RuntimeCommandSummaryTextElement do
        destination_attribute(:parent_id)
      end
    end
    ...
    ui_relationships do
      ...
      relationship :recovery_buttons do
        kind(:child)
        slot(:body)
        placement(:append)
        order(20)
    ...
    ui_element do
      ...
      props(%{
        title: "Runtime drill-down palette",
        description:
          "Keep the supervisor lane, process table, and supporting copy in one persisted runtime review state.",
        class: "ashui-example-command-palette"
      })

      metadata(%{id: "runtime-command-palette", section: "demo", slot: "body", position: 0})
    end
  end
  ...
  defmodule Examples.RuntimeCommandSearchInputElement do
    ...
    ui_element do
      ...
      props(%{
        ...
        type: "text",
        value: "",
        placeholder: "retry, backlog, rollback",
        class: "ashui-example-input"
      })

      metadata(%{id: "runtime-command-query", section: "demo", slot: "search", position: 0})
    end

    ui_bindings do
      binding :runtime_query do
        source(%{resource: "WorkspaceState", field: :runtime_query, id: "tutorial-services-incidents-state"})
        target("runtime_query")
        binding_type(:value)
        transform(%{})
        metadata(%{owner: "runtime_query", owner_signal: "change"})
      end
    end
  end
  ...
  defmodule Examples.FocusGatewayRuntimeButtonElement do
    ...
    ui_element do
      type(:button)
      props(%{label: "Gateway supervisor", class: "ashui-example-primary-cta", variant: "secondary"})
      metadata(%{id: "focus-gateway-runtime", section: "demo", slot: "body", position: 0})
    end

    ui_actions do
      action :focus_gateway_runtime do
        signal(:click)
        source(%{id: "tutorial-services-incidents-state", resource: "WorkspaceState", action: "update"})
        target("submit")
        ...
        transform(%{
          params: %{
            selected_value: %{"from" => "static", "value" => "services"},
            runtime_focus: %{"from" => "static", "value" => "gateway supervisor"},
            runtime_query: %{"from" => "static", "value" => "retry"},
            runtime_supervision_model: %{
              "from" => "static",
              "value" => %{
                "label" => "Gateway supervisor",
            ...
            runtime_support_title: %{"from" => "static", "value" => "Gateway supervisor lane"},
            runtime_support_notice: %{
              "from" => "static",
              "value" =>
                "This runtime tree is tutorial-seeded: the labels and restart counts are representative operator-facing snapshots, not a hidden live supervisor tap."
            ...
            detail_title: %{"from" => "static", "value" => "Gateway supervisor lane"},
            detail_summary: %{
              "from" => "static",
              "value" =>
                "Runtime drill-down is centered on the ingress supervisors, retry workers, and the persistence refresh lane behind the sev-1 review."
            },
            detail_status: %{"from" => "static", "value" => "watching"},
            status: %{"from" => "static", "value" => "Gateway supervisor drill-down loaded into the services workspace."}
          }
        })

        metadata(%{intent: "load_runtime_view", success_message: "Gateway runtime view loaded"})
      end
    end
  end
  ...
  defmodule Examples.FocusSearchRuntimeButtonElement do
    ...
    ui_element do
      type(:button)
      props(%{label: "Search recovery", class: "ashui-example-secondary-cta", variant: "secondary"})
      metadata(%{id: "focus-search-runtime", section: "demo", slot: "body", position: 10})
    end

    ui_actions do
      action :focus_search_runtime do
        signal(:click)
        source(%{id: "tutorial-services-incidents-state", resource: "WorkspaceState", action: "update"})
        target("submit")
        ...
        transform(%{
          params: %{
            selected_value: %{"from" => "static", "value" => "services"},
            runtime_focus: %{"from" => "static", "value" => "search replica supervision"},
            runtime_query: %{"from" => "static", "value" => "backlog"},
            runtime_supervision_model: %{
              "from" => "static",
              "value" => %{
                "label" => "Search replica supervisor",
            ...
            runtime_support_title: %{"from" => "static", "value" => "Replica recovery lane"},
            runtime_support_notice: %{
              "from" => "static",
              "value" =>
                "This view stays explicit about being sampled: the backlog workers and promotion guard represent the operator's captured review packet, not a direct runtime probe."
            ...
            detail_title: %{"from" => "static", "value" => "Search recovery lane"},
            detail_summary: %{
              "from" => "static",
              "value" =>
                "Runtime drill-down is centered on the replica supervisor, the queued backlog shards, and the promotion guard that gates recovery."
            },
            detail_status: %{"from" => "static", "value" => "recovering"},
            status: %{"from" => "static", "value" => "Search recovery runtime drill-down loaded into the services workspace."}
          }
        })

        metadata(%{intent: "load_runtime_view", success_message: "Search runtime view loaded"})
      end
    end
  end
  ...
  defmodule Examples.FocusRecoveryRuntimeButtonElement do
    ...
    ui_element do
      type(:button)
      props(%{label: "Rollback coordinator", class: "ashui-example-secondary-cta", variant: "secondary"})
      metadata(%{id: "focus-recovery-runtime", section: "demo", slot: "body", position: 20})
    end

    ui_actions do
      action :focus_recovery_runtime do
        signal(:click)
        source(%{id: "tutorial-services-incidents-state", resource: "WorkspaceState", action: "update"})
        target("submit")
        ...
        transform(%{
          params: %{
            selected_value: %{"from" => "static", "value" => "services"},
            runtime_focus: %{"from" => "static", "value" => "rollback coordination"},
            runtime_query: %{"from" => "static", "value" => "notify"},
            runtime_supervision_model: %{
              "from" => "static",
              "value" => %{
                "label" => "Rollback coordinator",
            ...
            runtime_support_title: %{"from" => "static", "value" => "Recovery coordination lane"},
            runtime_support_notice: %{
              "from" => "static",
              "value" =>
                "This recovery tree is deliberately operator-shaped. It teaches how to present rollback coordination honestly without implying a hidden orchestration runtime inside the tutorial app."
            ...
            detail_title: %{"from" => "static", "value" => "Rollback coordination lane"},
            detail_summary: %{
              "from" => "static",
              "value" =>
                "Runtime drill-down is centered on the rollback packet supervisor, the queued notifications, and the maintenance guard that blocks restart confirmation."
            },
            detail_status: %{"from" => "static", "value" => "active"},
            status: %{"from" => "static", "value" => "Rollback coordination runtime drill-down loaded into the services workspace."}
          }
        })

        metadata(%{intent: "load_runtime_view", success_message: "Recovery runtime view loaded"})
      end
    end
  end
  ...
  defmodule Examples.RuntimeCommandSummaryTextElement do
    ...
    ui_element do
      type(:text)
      props(%{content: "", class: "ashui-tutorial-muted-copy"})
      metadata(%{id: "runtime-command-summary", section: "demo", slot: "footer", position: 0})
    end

    ui_bindings do
      binding :runtime_command_summary do
        source(%{resource: "WorkspaceState", field: :runtime_command_summary, id: "tutorial-services-incidents-state"})
        target("content")
        binding_type(:value)
        transform(%{})
        metadata(%{owner: "runtime_command_summary"})
      end
    end
  end
  ...
  defmodule Examples.RuntimeTabsElement do
    ...
    relationships do
      has_many :gateway_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowGatewayRuntimeTabButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :search_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowSearchRuntimeTabButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :recovery_buttons,
               AshUITutorials.ProductionPolish.Examples.ShowRecoveryRuntimeTabButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :status_texts,
               AshUITutorials.ProductionPolish.Examples.RuntimeTabsStatusTextElement do
        destination_attribute(:parent_id)
      end
    end
    ...
    ui_element do
      ...
      props(%{
        title: "Runtime lanes",
        description:
          "Tabs keep gateway, search, and rollback supervision stories visible without discarding the rest of the services workspace.",
        class: "ashui-tutorial-topology-tabs"
      })

      metadata(%{id: "runtime-tabs", section: "demo", slot: "body", position: 10})
    end
  end
  ...
  defmodule Examples.ShowGatewayRuntimeTabButtonElement do
    ...
    ui_element do
      type(:button)
      props(%{label: "Gateway", class: "ashui-example-nav-button", variant: "secondary"})
      metadata(%{id: "show-gateway-runtime-tab", section: "demo", slot: "nav", position: 0})
    end

    ui_actions do
      action :show_gateway_runtime_tab do
        signal(:click)
        source(%{id: "tutorial-services-incidents-state", resource: "WorkspaceState", action: "update"})
        target("submit")
        ...
        transform(%{
          params: %{
            runtime_focus: %{"from" => "static", "value" => "gateway supervisor"},
            runtime_status_copy: %{"from" => "static", "value" => "Runtime review is focused on the gateway supervisor lane and the retry workers behind the sev-1 ingress path."},
            status: %{"from" => "static", "value" => "Gateway runtime lane selected."}
          }
        })

        metadata(%{intent: "select_runtime_tab", success_message: "Gateway runtime lane selected"})
      end
    end
  end
  ...
  defmodule Examples.ShowSearchRuntimeTabButtonElement do
    ...
    ui_element do
      type(:button)
      props(%{label: "Search", class: "ashui-example-nav-button", variant: "secondary"})
      metadata(%{id: "show-search-runtime-tab", section: "demo", slot: "nav", position: 10})
    end

    ui_actions do
      action :show_search_runtime_tab do
        signal(:click)
        source(%{id: "tutorial-services-incidents-state", resource: "WorkspaceState", action: "update"})
        target("submit")
        ...
        transform(%{
          params: %{
            runtime_focus: %{"from" => "static", "value" => "search replica supervision"},
            runtime_status_copy: %{"from" => "static", "value" => "Runtime review is narrowed to the search recovery supervisor and the queued backlog workers."},
            status: %{"from" => "static", "value" => "Search runtime lane selected."}
          }
        })

        metadata(%{intent: "select_runtime_tab", success_message: "Search runtime lane selected"})
      end
    end
  end
  ...
  defmodule Examples.ShowRecoveryRuntimeTabButtonElement do
    ...
    ui_element do
      type(:button)
      props(%{label: "Recovery", class: "ashui-example-nav-button", variant: "secondary"})
      metadata(%{id: "show-recovery-runtime-tab", section: "demo", slot: "nav", position: 20})
    end

    ui_actions do
      action :show_recovery_runtime_tab do
        signal(:click)
        source(%{id: "tutorial-services-incidents-state", resource: "WorkspaceState", action: "update"})
        target("submit")
        ...
        transform(%{
          params: %{
            runtime_focus: %{"from" => "static", "value" => "rollback coordination"},
            runtime_status_copy: %{"from" => "static", "value" => "Runtime review is focused on the rollback coordination lane and the guarded recovery notifications behind it."},
            status: %{"from" => "static", "value" => "Recovery runtime lane selected."}
          }
        })

        metadata(%{intent: "select_runtime_tab", success_message: "Recovery runtime lane selected"})
      end
    end
  end
  ...
  defmodule Examples.RuntimeTabsStatusTextElement do
    ...
    ui_element do
      type(:text)
      props(%{content: "", class: "ashui-example-surface-copy ashui-example-tabs-panel"})
      metadata(%{id: "runtime-tabs-status", section: "demo", slot: "body", position: 0})
    end

    ui_bindings do
      binding :runtime_focus do
        source(%{resource: "WorkspaceState", field: :runtime_focus, id: "tutorial-services-incidents-state"})
        target("content")
        binding_type(:value)
        transform(%{})
        metadata(%{owner: "runtime_focus"})
      end
    end
  end
  ...
  defmodule Examples.RuntimeSupportPanelElement do
    ...
    relationships do
      has_many :titles,
               AshUITutorials.ProductionPolish.Examples.RuntimeSupportTitleElement do
        destination_attribute(:parent_id)
      end

      has_many :notices,
               AshUITutorials.ProductionPolish.Examples.RuntimeSupportNoticeElement do
        destination_attribute(:parent_id)
      end
    end
    ...
    ui_element do
      type(:card)
      props(%{class: "ashui-example-layout-card ashui-tutorial-runtime-support"})
      metadata(%{id: "runtime-support-panel", section: "demo", slot: "body", position: 20})
    end
  end
  ...
  defmodule Examples.RuntimeSupportTitleElement do
    ...
    ui_element do
      ...
      metadata(%{id: "runtime-support-title", section: "demo", slot: "body", position: 0})
    end

    ui_bindings do
      binding :runtime_support_title do
        source(%{resource: "WorkspaceState", field: :runtime_support_title, id: "tutorial-services-incidents-state"})
        target("content")
        binding_type(:value)
        transform(%{})
        metadata(%{owner: "runtime_support_title"})
      end
    end
  end
  ...
  defmodule Examples.RuntimeSupportNoticeElement do
    ...
    ui_element do
      type(:text)
      props(%{content: "", class: "ashui-tutorial-muted-copy"})
      metadata(%{id: "runtime-support-notice", section: "demo", slot: "body", position: 10})
    end

    ui_bindings do
      binding :runtime_support_notice do
        source(%{resource: "WorkspaceState", field: :runtime_support_notice, id: "tutorial-services-incidents-state"})
        target("content")
        binding_type(:value)
        transform(%{})
        metadata(%{owner: "runtime_support_notice"})
      end
    end
  end
  ...
  defmodule Examples.RuntimeSupervisionTreeViewerElement do
    ...
    ui_element do
      ...
      props(%{
        ...
        description: "Expose the active runtime tree explicitly, including which parts are sampled, stale, or tutorial-seeded.",
        class: "ashui-example-supervision-tree-shell"
      })

      metadata(%{id: "runtime-supervision-tree", section: "demo", slot: "body", position: 30})
    end

    ui_bindings do
      binding :runtime_supervision_model do
        source(%{resource: "WorkspaceState", field: :runtime_supervision_model, id: "tutorial-services-incidents-state"})
        target("model")
        binding_type(:value)
        transform(%{})
        metadata(%{owner: "runtime_supervision_model"})
      end
    end
  end
  ...
  defmodule Examples.RuntimeProcessTableElement do
    ...
    ui_element do
      ...
      props(%{
        title: "Process review table",
        description: "Keep the current process rows visible beside the supervision tree so operators can cross-check state, detail, and restart posture.",
        columns: [
      ...
      metadata(%{id: "runtime-process-table", section: "demo", slot: "body", position: 40})
    end

    ui_bindings do
      binding :runtime_process_rows do
        source(%{resource: "WorkspaceState", field: :runtime_process_rows, id: "tutorial-services-incidents-state"})
        target("items")
        binding_type(:list)
        transform(%{})
        metadata(%{owner: "runtime_process_rows"})
      end
    end
  end
  ...
  defmodule Examples.RuntimeStatusTextElement do
    ...
    ui_element do
      type(:text)
      props(%{content: "", class: "ashui-tutorial-muted-copy"})
      metadata(%{id: "runtime-status-text", section: "demo", slot: "footer", position: 0})
    end

    ui_bindings do
      binding :runtime_status_copy do
        source(%{resource: "WorkspaceState", field: :runtime_status_copy, id: "tutorial-services-incidents-state"})
        target("content")
        binding_type(:value)
        transform(%{})
        metadata(%{owner: "runtime_status_copy"})
      end
    end
  end
  ...
  defmodule Examples.RolePolicySummaryPanelElement do
    ...
    relationships do
      has_many :name_texts,
               AshUITutorials.ProductionPolish.Examples.RolePolicyNameTextElement do
        destination_attribute(:parent_id)
      end

      has_many :summary_texts,
               AshUITutorials.ProductionPolish.Examples.RolePolicySummaryTextElement do
        destination_attribute(:parent_id)
      end

      has_many :notice_texts,
               AshUITutorials.ProductionPolish.Examples.RolePolicyNoticeTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.AdminPolicyAuditPanelElement do
    ...
    relationships do
      has_many :texts,
               AshUITutorials.ProductionPolish.Examples.AdminPolicyAuditTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.ViewerPolicyNoticePanelElement do
    ...
    relationships do
      has_many :texts,
               AshUITutorials.ProductionPolish.Examples.ViewerPolicyNoticeTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.SharedDetailCardElement do
    ...
    relationships do
      has_many :badges, AshUITutorials.ProductionPolish.Examples.SharedDetailBadgeElement do
        destination_attribute(:parent_id)
      end

      has_many :titles, AshUITutorials.ProductionPolish.Examples.SharedDetailTitleElement do
        destination_attribute(:parent_id)
      end

      has_many :summaries, AshUITutorials.ProductionPolish.Examples.SharedDetailSummaryElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.IncidentsWorkspacePanelElement do
    ...
    relationships do
      ...
      has_many :filter_groups,
               AshUITutorials.ProductionPolish.Examples.IncidentsFiltersGroupElement do
        destination_attribute(:parent_id)
      end

      has_many :production_polish_panels,
               AshUITutorials.ProductionPolish.Examples.IncidentsProductionPolishPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :operator_form_panels,
               AshUITutorials.ProductionPolish.Examples.OperatorFormsPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :guarded_action_panels,
               AshUITutorials.ProductionPolish.Examples.GuardedActionsPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :admin_policy_panels,
               AshUITutorials.ProductionPolish.Examples.AdminPolicyAuditPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :viewer_policy_panels,
               AshUITutorials.ProductionPolish.Examples.ViewerPolicyNoticePanelElement do
        destination_attribute(:parent_id)
      end

      has_many :incident_tables,
               AshUITutorials.ProductionPolish.Examples.IncidentsTableElement do
        destination_attribute(:parent_id)
      end

      has_many :runbook_review_panels,
               AshUITutorials.ProductionPolish.Examples.RunbookReviewPanelElement do
        destination_attribute(:parent_id)
      end

      has_many :topology_and_navigation_panels,
               AshUITutorials.ProductionPolish.Examples.LiveDiagnosticsPanelElement do
        destination_attribute(:parent_id)
      end
      ...
      has_many :status_texts, AshUITutorials.ProductionPolish.Examples.IncidentsStatusTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.IncidentsFiltersGroupElement do
    ...
    relationships do
      has_many :severity_fields,
               AshUITutorials.ProductionPolish.Examples.IncidentSeverityFieldElement do
        destination_attribute(:parent_id)
      end

      has_many :escalated_fields,
               AshUITutorials.ProductionPolish.Examples.IncidentEscalatedFieldElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.IncidentSeverityFieldElement do
    ...
    relationships do
      has_many :radios,
               AshUITutorials.ProductionPolish.Examples.IncidentSeverityRadioElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.IncidentEscalatedFieldElement do
    ...
    relationships do
      has_many :switches,
               AshUITutorials.ProductionPolish.Examples.IncidentEscalatedSwitchElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.OperatorFormsPanelElement do
    ...
    relationships do
      has_many :forms, AshUITutorials.ProductionPolish.Examples.OperatorWorkflowFormElement do
        destination_attribute(:parent_id)
      end

      has_many :feedback_badges,
               AshUITutorials.ProductionPolish.Examples.FormFeedbackBadgeElement do
        destination_attribute(:parent_id)
      end

      has_many :feedback_titles,
               AshUITutorials.ProductionPolish.Examples.FormFeedbackTitleElement do
        destination_attribute(:parent_id)
      end

      has_many :feedback_summaries,
               AshUITutorials.ProductionPolish.Examples.FormFeedbackSummaryElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.OperatorWorkflowFormElement do
    ...
    relationships do
      has_many :note_groups,
               AshUITutorials.ProductionPolish.Examples.NoteAndAssignmentGroupElement do
        destination_attribute(:parent_id)
      end

      has_many :maintenance_groups,
               AshUITutorials.ProductionPolish.Examples.MaintenanceWindowGroupElement do
        destination_attribute(:parent_id)
      end

      has_many :acknowledge_buttons,
               AshUITutorials.ProductionPolish.Examples.AcknowledgeIncidentButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :assign_buttons,
               AshUITutorials.ProductionPolish.Examples.AssignIncidentButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :maintenance_buttons,
               AshUITutorials.ProductionPolish.Examples.ScheduleMaintenanceButtonElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.NoteAndAssignmentGroupElement do
    ...
    relationships do
      has_many :note_fields,
               AshUITutorials.ProductionPolish.Examples.OperatorNoteFieldElement do
        destination_attribute(:parent_id)
      end

      has_many :assignment_fields,
               AshUITutorials.ProductionPolish.Examples.AssignmentTargetFieldElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.OperatorNoteFieldElement do
    ...
    relationships do
      has_many :inputs,
               AshUITutorials.ProductionPolish.Examples.OperatorNoteInputElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.AssignmentTargetFieldElement do
    ...
    relationships do
      has_many :pick_lists,
               AshUITutorials.ProductionPolish.Examples.AssignmentTargetPickListElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.MaintenanceWindowGroupElement do
    ...
    relationships do
      has_many :duration_fields,
               AshUITutorials.ProductionPolish.Examples.MaintenanceDurationFieldElement do
        destination_attribute(:parent_id)
      end

      has_many :date_fields,
               AshUITutorials.ProductionPolish.Examples.MaintenanceDateFieldElement do
        destination_attribute(:parent_id)
      end

      has_many :time_fields,
               AshUITutorials.ProductionPolish.Examples.MaintenanceTimeFieldElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.MaintenanceDurationFieldElement do
    ...
    relationships do
      has_many :inputs,
               AshUITutorials.ProductionPolish.Examples.MaintenanceDurationInputElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.MaintenanceDateFieldElement do
    ...
    relationships do
      has_many :inputs,
               AshUITutorials.ProductionPolish.Examples.MaintenanceDateInputElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.MaintenanceTimeFieldElement do
    ...
    relationships do
      has_many :inputs,
               AshUITutorials.ProductionPolish.Examples.MaintenanceTimeInputElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.GuardedActionsPanelElement do
    ...
    relationships do
      has_many :menus, AshUITutorials.ProductionPolish.Examples.GuardedActionsMenuElement do
        destination_attribute(:parent_id)
      end

      has_many :overlays, AshUITutorials.ProductionPolish.Examples.GuardOverlayElement do
        destination_attribute(:parent_id)
      end

      has_many :dialogs, AshUITutorials.ProductionPolish.Examples.ResolveGuardDialogElement do
        destination_attribute(:parent_id)
      end

      has_many :alerts, AshUITutorials.ProductionPolish.Examples.RestartGuardAlertElement do
        destination_attribute(:parent_id)
      end

      has_many :toasts, AshUITutorials.ProductionPolish.Examples.GuardResultToastElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.GuardedActionsMenuElement do
    ...
    relationships do
      has_many :resolve_buttons,
               AshUITutorials.ProductionPolish.Examples.OpenResolveGuardButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :restart_buttons,
               AshUITutorials.ProductionPolish.Examples.OpenRestartGuardButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :silence_buttons,
               AshUITutorials.ProductionPolish.Examples.OpenSilenceGuardButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :discard_buttons,
               AshUITutorials.ProductionPolish.Examples.OpenDiscardNoteGuardButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :summary_texts,
               AshUITutorials.ProductionPolish.Examples.GuardSummaryTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.GuardOverlayElement do
    ...
    relationships do
      has_many :title_texts,
               AshUITutorials.ProductionPolish.Examples.GuardOverlayTitleTextElement do
        destination_attribute(:parent_id)
      end

      has_many :summary_texts,
               AshUITutorials.ProductionPolish.Examples.GuardOverlaySummaryTextElement do
        destination_attribute(:parent_id)
      end

      has_many :confirm_buttons,
               AshUITutorials.ProductionPolish.Examples.ConfirmOverlayGuardButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :cancel_buttons,
               AshUITutorials.ProductionPolish.Examples.CancelGuardSurfaceButtonElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.ResolveGuardDialogElement do
    ...
    relationships do
      has_many :summary_texts,
               AshUITutorials.ProductionPolish.Examples.ResolveGuardSummaryTextElement do
        destination_attribute(:parent_id)
      end

      has_many :confirm_buttons,
               AshUITutorials.ProductionPolish.Examples.ConfirmResolveGuardButtonElement do
        destination_attribute(:parent_id)
      end
  ...
  defmodule Examples.RestartGuardAlertElement do
    ...
    relationships do
      has_many :summary_texts,
               AshUITutorials.ProductionPolish.Examples.RestartGuardSummaryTextElement do
        destination_attribute(:parent_id)
      end

      has_many :confirm_buttons,
               AshUITutorials.ProductionPolish.Examples.ConfirmRestartGuardButtonElement do
        destination_attribute(:parent_id)
      end
  ...
  defmodule Examples.GuardResultToastElement do
    ...
    relationships do
      has_many :title_texts,
               AshUITutorials.ProductionPolish.Examples.GuardToastTitleTextElement do
        destination_attribute(:parent_id)
      end

      has_many :summary_texts,
               AshUITutorials.ProductionPolish.Examples.GuardToastSummaryTextElement do
        destination_attribute(:parent_id)
      end

      has_many :dismiss_buttons,
               AshUITutorials.ProductionPolish.Examples.DismissGuardToastButtonElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.RunbookReviewPanelElement do
    ...
    relationships do
      has_many :split_panes,
               AshUITutorials.ProductionPolish.Examples.RunbookSplitPaneElement do
        destination_attribute(:parent_id)
      end

      has_many :status_texts,
               AshUITutorials.ProductionPolish.Examples.RunbookStatusTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.RunbookSplitPaneElement do
    ...
    relationships do
      has_many :focus_titles,
               AshUITutorials.ProductionPolish.Examples.RunbookFocusTitleElement do
        destination_attribute(:parent_id)
      end

      has_many :markdown_viewers,
               AshUITutorials.ProductionPolish.Examples.RunbookMarkdownViewerElement do
        destination_attribute(:parent_id)
      end

      has_many :evidence_cards,
               AshUITutorials.ProductionPolish.Examples.AttachmentEvidenceCardElement do
        destination_attribute(:parent_id)
      end

      has_many :gateway_buttons,
               AshUITutorials.ProductionPolish.Examples.LoadGatewayRunbookButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :rollback_buttons,
               AshUITutorials.ProductionPolish.Examples.LoadRollbackRunbookButtonElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.AttachmentEvidenceCardElement do
    ...
    relationships do
      has_many :preview_texts,
               AshUITutorials.ProductionPolish.Examples.AttachmentPreviewTextElement do
        destination_attribute(:parent_id)
      end

      has_many :file_fields,
               AshUITutorials.ProductionPolish.Examples.AttachmentFileFieldElement do
        destination_attribute(:parent_id)
      end

      has_many :reference_links,
               AshUITutorials.ProductionPolish.Examples.AttachmentReferenceLinkElement do
        destination_attribute(:parent_id)
      end

      has_many :images, AshUITutorials.ProductionPolish.Examples.AttachmentImageElement do
        destination_attribute(:parent_id)
      end

      has_many :support_texts,
               AshUITutorials.ProductionPolish.Examples.AttachmentSupportTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.AttachmentFileFieldElement do
    ...
    relationships do
      has_many :inputs,
               AshUITutorials.ProductionPolish.Examples.AttachmentFileInputElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.LiveDiagnosticsPanelElement do
    ...
    relationships do
      has_many :gateway_buttons,
               AshUITutorials.ProductionPolish.Examples.LoadGatewayDiagnosticsButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :search_buttons,
               AshUITutorials.ProductionPolish.Examples.LoadSearchDiagnosticsButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :pressure_buttons,
               AshUITutorials.ProductionPolish.Examples.LoadPressureDiagnosticsButtonElement do
        destination_attribute(:parent_id)
      end

      has_many :status_surfaces,
               AshUITutorials.ProductionPolish.Examples.DiagnosticsStatusElement do
        destination_attribute(:parent_id)
      end

      has_many :feedback_surfaces,
               AshUITutorials.ProductionPolish.Examples.DiagnosticsInlineFeedbackElement do
        destination_attribute(:parent_id)
      end

      has_many :log_viewers,
               AshUITutorials.ProductionPolish.Examples.DiagnosticsLogViewerElement do
        destination_attribute(:parent_id)
      end

      has_many :stream_widgets,
               AshUITutorials.ProductionPolish.Examples.DiagnosticsStreamWidgetElement do
        destination_attribute(:parent_id)
      end

      has_many :process_monitors,
               AshUITutorials.ProductionPolish.Examples.DiagnosticsProcessMonitorElement do
        destination_attribute(:parent_id)
      end

      has_many :status_texts,
               AshUITutorials.ProductionPolish.Examples.DiagnosticsStatusTextElement do
        destination_attribute(:parent_id)
      end
    end
  ...
  defmodule Examples.ServicesScreen do
    ...
    relationships do
      has_many :panels, AshUITutorials.ProductionPolish.Examples.ServicesWorkspacePanelElement do
        destination_attribute(:screen_id)
      end

      has_many :story_texts, AshUITutorials.ProductionPolish.Examples.ServicesStoryTextElement do
        destination_attribute(:screen_id)
      end

      has_many :signal_texts, AshUITutorials.ProductionPolish.Examples.ServicesSignalTextElement do
        destination_attribute(:screen_id)
      end
    end
    ...
    ui_screen do
      ...
      metadata(%{
        ...
        tutorial_directory: "12-production-polish",
        shell_id: "production-polish-services-shell"
      })
    end
  end
  ...
  defmodule Examples.IncidentsScreen do
    ...
    relationships do
      has_many :panels, AshUITutorials.ProductionPolish.Examples.IncidentsWorkspacePanelElement do
        destination_attribute(:screen_id)
      end

      has_many :story_texts, AshUITutorials.ProductionPolish.Examples.IncidentsStoryTextElement do
        destination_attribute(:screen_id)
      end

      has_many :signal_texts, AshUITutorials.ProductionPolish.Examples.IncidentsSignalTextElement do
        destination_attribute(:screen_id)
      end
    end
    ...
    ui_screen do
      ...
      metadata(%{
        ...
        tutorial_directory: "12-production-polish",
        shell_id: "production-polish-incidents-shell"
      })
    end
  end

  defmodule ExampleSeeds do
    def seed!(opts \\ []), do: AshUITutorials.ProductionPolish.seed!(opts)
    def reset!, do: AshUITutorials.ProductionPolish.reset!()
  end
  ...
  defmodule Web.Router do
    ...
    scope "/", AshUITutorials.ProductionPolish.Web do
  ...
  defmodule Web.Endpoint do
    use Phoenix.Endpoint, otp_app: :ash_ui_tutorial_production_polish

    @session_options [
      store: :cookie,
      key: "_ash_ui_tutorial_production_polish_key",
    ...
    plug(AshUITutorials.ProductionPolish.Web.Router)
  end
      ...
      <main class="ashui-example-shell">
        ...
        <header class="ashui-tutorial-shell-header">
          ...
          <nav class="ashui-tutorial-nav" aria-label="Primary workspace navigation">
            <a
              href={AshUITutorials.ProductionPolish.page_path("services", @current_actor, @example_runtime)}
              class={["ashui-tutorial-nav-link", @active_page == "services" && "is-active"]}
            >
              Services
            </a>
            <a
              href={AshUITutorials.ProductionPolish.page_path("incidents", @current_actor, @example_runtime)}
          ...
          <section class="ashui-tutorial-actor-switcher" aria-label="Actor preview">
            ...
            <div class="ashui-tutorial-actor-links">
              <%= for actor <- @actor_profiles do %>
                <a
                  href={AshUITutorials.ProductionPolish.page_path(@active_page, actor, @example_runtime)}
                  class={[
                ...
                >
                  <%= AshUITutorials.ProductionPolish.actor_switch_label(actor) %>
                </a>
              <% end %>
            </div>
          </section>
  ...
  defmodule Web.ServicesLive do
    ...
    alias AshUITutorials.ProductionPolish.Web.Components.TutorialShell
    alias AshUI.LiveView.EventHandler
    alias AshUI.LiveView.Integration

    def mount(params, _session, socket) do
      actor = AshUITutorials.ProductionPolish.actor_profile_value(params["actor"])
      AshUITutorials.ProductionPolish.seed!(mount_actor: actor)
    ...
    defp mount_screen(socket, params, screen_kind, actor) do
      ...
      actor = AshUITutorials.ProductionPolish.actor_profile_value(actor)

      socket =
        socket
        |> Phoenix.Component.assign(:current_user, actor)
        |> Phoenix.Component.assign(:current_actor, actor)
        |> Phoenix.Component.assign(:actor_profiles, AshUITutorials.ProductionPolish.actor_profiles())
        |> Phoenix.Component.assign(:ash_ui_storage, AshUITutorials.ProductionPolish.ui_storage())
        |> Phoenix.Component.assign(:ash_ui_domains, AshUITutorials.ProductionPolish.runtime_domains())
        |> Phoenix.Component.assign(:page_title, AshUITutorials.ProductionPolish.title())
        |> Phoenix.Component.assign(:theme_css, AshUITutorials.ProductionPolish.theme_css())
        |> Phoenix.Component.assign(:example_runtime, example_runtime)
        |> Phoenix.Component.assign(:supported_runtimes, AshUITutorials.ProductionPolish.supported_runtimes())
        |> Phoenix.Component.assign(:active_page, Atom.to_string(screen_kind))

      with {:ok, socket} <- Integration.mount_ui_screen(socket, AshUITutorials.ProductionPolish.screen_name(screen_kind), params),
    ...
    defp render_workspace(assigns, active_page) do
      assigns =
        ...
        |> Phoenix.Component.assign_new(:supported_runtimes, fn -> AshUITutorials.ProductionPolish.supported_runtimes() end)
        |> Phoenix.Component.assign_new(:example_runtime, fn -> AshUITutorials.ProductionPolish.default_runtime() end)
        |> Phoenix.Component.assign_new(:current_actor, fn -> AshUITutorials.ProductionPolish.current_user() end)
        |> Phoenix.Component.assign_new(:actor_profiles, fn -> AshUITutorials.ProductionPolish.actor_profiles() end)
        |> Phoenix.Component.assign_new(:rendered_runtime, fn ->
          %{
            content: assigns[:rendered_ui] || "",
            description: AshUITutorials.ProductionPolish.runtime_description(AshUITutorials.ProductionPolish.default_runtime()),
            mode: :live_fragment,
            runtime: AshUITutorials.ProductionPolish.default_runtime()
          }
        end)
      ...
      <TutorialShell.tutorial_shell
        title={@page_title}
        summary={AshUITutorials.ProductionPolish.summary()}
        theme_css={@theme_css}
      ...
      >
          ...
          <div class="ashui-tutorial-runtime-actions">
            ...
            <nav class="ashui-tutorial-runtime-links" aria-label="Runtime preview selection">
              <%= for runtime <- @supported_runtimes do %>
                <a
                  href={AshUITutorials.ProductionPolish.runtime_path(@active_page, @current_actor, runtime)}
                  class={[
                    "ashui-tutorial-nav-link",
        ...
        <section class="ashui-tutorial-runtime-view">
          <%= case @rendered_runtime.mode do %>
            <% :html_document -> %>
              <iframe class="ashui-tutorial-runtime-frame" sandbox="allow-same-origin" srcdoc={@rendered_runtime.content} title={"production-polish-#{@rendered_runtime.runtime}"} />
    ...
    defp refresh_rendered_ui(socket) do
      rendered_runtime =
        AshUITutorials.ProductionPolish.rendered_runtime(
          socket.assigns,
          socket.assigns[:example_runtime] || AshUITutorials.ProductionPolish.default_runtime()
        )
    ...
    defp runtime_from_params(params) do
      params["runtime"]
      |> fallback_runtime()
      |> AshUITutorials.ProductionPolish.normalize_runtime!()
    end
